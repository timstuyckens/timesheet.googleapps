define(["knockout","utils"], function(ko,utils) {
	...
	that.getOverviewByEventsAndCategories=function(items,cats,theDate){
		var now=theDate || new Date(),	temp=[];
		ko.utils.arrayForEach(cats, function(i) {	
			var aantalOpgenomen=0, aantalGepland=0,name=i.name(),max=i.number(),remaining=0;
			...
			temp.push({name:name,max:max,aantalOpgenomen:aantalOpgenomen,aantalGepland:aantalGepland,remaining:remaining});
		});
		return temp;
	};
	
	that.getGroupedOverviewByEventsAndCategories=function(items,cats,theDate){
		var now=theDate || new Date(),temp=[{name:"Detail voorbijgaande",items:[]},{name:"Detail toekomst",items:[]}];
		ko.utils.arrayForEach(items, function(item) {
